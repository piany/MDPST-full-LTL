    ...
    for k, L in enumerate(LL):
        ...
        for n, l in enumerate(L):
            ...
            if ip_count >= 1:
                ...
                break
    print('Analyze done')
    print('Total %s simulations: %s failure (%s) (%s prefix), %s prefix successful' % (str(N), str(failure_count), str(failure_count/N),
          str(prefix_failure_count), str(prefix_suc_count)))
...
def visualize_world(WS_d, WS_node_dict, name=None):
    ...
    for node, prop in WS_node_dict.items():
        if frozenset(['base1']) in list(prop.keys()):
            text = '$Base1$'
            color = 'yellow'
        elif frozenset(['base2']) in list(prop.keys()):
            text = '$Base2$'
            color = 'yellow'
        elif frozenset(['base3']) in list(prop.keys()):
            text = '$Base3$'
            color = 'yellow'
        elif frozenset(['obstacle']) in list(prop.keys()):
            text = '$Obs$'
            color = 'grey'
        elif frozenset(['obstacle', ]) in list(prop.keys()):
            text = '$Obs: 1.0$'
        ...
        #                                    ls='--',
        #                                    alpha=0.8)
        vertices=[(node[0]-1, node[1]), (node[0]-1/2, node[1]-math.sqrt(3)/2), (node[0]+1/2, node[1]-math.sqrt(3)/2), (node[0]+1, node[1]), (node[0]+1/2, node[1]+math.sqrt(3)/2), (node[0]-1/2, node[1]+math.sqrt(3)/2)]
        rec=Polygon(vertices, 
                    fill=True,
                    facecolor=color,
                    edgecolor='black',
                    linewidth=1,
        ...
        ax.add_patch(rec)
        if text:
            ax.text(node[0]-0.7, node[1]-0.2, r'%s' %
...
def visualize_world_paths(WS_d, WS_node_dict, XX, LL, UU, MM, name=None):
    ...
    for n in range(0, N):
        ...
        for k in range(0, K):
            ...
            # ----
            #if (M[k] == 1) and (k<= K-2):
            norm = Normalize(vmin=0, vmax=2000)  # Normalize the time steps to fit color map
        ...
        sm.set_array([])  # This is required for the color bar
