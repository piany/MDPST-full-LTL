...
#------------------Build corresponding MDP from MDPST-----------------
new_var = DiGraph
class Motion_MDP_from_MDPST(new_var):
    # -------------------------------
    def __init__(self, node_dict, edge_dict, U, initial_node, initial_label):
        DiGraph.__init__(self, name='motion_mdp',
                         init_state=initial_node, init_label=initial_label)
        for (n, prob_label) in node_dict.items():
            self.add_node(n, label=prob_label, act=set())
        print("-------Motion MDP Initialized-------")
        self.add_edges(node_dict, edge_dict, U)
        print("%s states and %s edges" %
              (str(len(self.nodes())), str(len(self.edges()))))
        #self.unify_mdp()

    def add_edges(self, node_dict, edge_dict, U):
        self.graph['U'] = set()
        for u in U:
            self.graph['U'].add(tuple(u))
        for edge, attri in edge_dict.items():
            ...
            # print(len(t_node))
            if len(t_node) > 1:
                for k, tt_node in enumerate(t_node):
                    prob_cost = dict()
                    prob_cost[tuple(u)] = [attri[0]/len(t_node), attri[1]/len(t_node)]
                    self.add_edge(f_node, tt_node, prop=prob_cost)
            else:
                tt_node = t_node[0]
                if (f_node, tt_node) in self.edges():
                    prop = self[f_node][tt_node]['prop']
                    prop[tuple(u)] = [attri[0], attri[1]]
                else:
                    prob_cost = dict()
                    #--- prob, cost
        ...
        for T in MEC:
            ...
            for s_f in T_temp:
                if s_f not in simple_digraph:
                    simple_digraph.add_node(s_f)
                for u in A[s_f]:
                    s_t_set = mdp.state_action[(s_f, u)]
                    if s_t_set.issubset(T_temp):
                        for s_t in s_t_set:
                            simple_digraph.add_edge(s_f, s_t)
                # for s_t in mdp.successors(s_f):
                #     if s_t in T_temp:
