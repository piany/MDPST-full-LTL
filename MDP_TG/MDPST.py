...
def optimal_plan_prefix_mdpst(mdp, Acc, SR):
    ...
    num_iteration = 0
    delta_old = 1
    while delta_old >= 0.01 and num_iteration < 10:
...
class Product_MDPST_ldba:
                        ...
                        for t_mdp_label, t_label_prob in smdp.nodes[t_mdp_node].items():
                            ...
                            for u, attri in smdp.edges[(f_mdp_node, t_mdp_node_set)].items():
                                if attri[0] != 0:                      
                                    ...
                                    for t_mdp_node in smdp.state_action[(f_mdp_node, u)]:
                                        t_prod_node = (t_mdp_node, t_mdp_label, t_ldba_node)
                                        prod_state_action_state[(f_prod_node, u, t_prod_node)] = smdp.edges_prob[(f_mdp_node, u, t_mdp_node)]
    ...
    def execution(self, smdp, pre_node, best_prefix_plan, TS_MEC, suffix_action, total_T, state_seq, label_seq):
        ...
        while (t <= total_T):
            if (t == 0):
                ...
                for next_state_set in self.successors[prev_state]:
                    if ((prev_state, u, next_state_set) in list(self.prod_state_action_state.keys())):
                        ...
                        t2 = next_state_set[2]
                        for s in t0:
                            for t_mdp_label, t_label_prob in smdp.nodes[s].items():
                                t1 = t_mdp_label
                                next_state_valid = (s, t1, t2)
                                result.append(next_state_valid)
            ...
            # ----
            u, m, index_suffix_node = self.act_by_plan_mdpst(pre_node, best_prefix_plan, TS_MEC, suffix_action, current_state, index_suffix_node)
            print('action: %s' %u)
            X.append(mdp_state)
