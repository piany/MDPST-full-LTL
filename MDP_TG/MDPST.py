...
class Motion_MDPST:
    ...
    def add_edges(self, edge_dict):
        ...
        for edge, attri in edge_dict.items():
            ...
            if (f_node, t_node_set) in edges.keys():
                prob_cost = edges[(f_node, t_node_set)]
                prob_cost[tuple(u)] = [attri[0], attri[1]]
            else:
                prob_cost = dict()
                prob_cost[tuple(u)] = [attri[0], attri[1]]
                edges[(f_node, t_node_set)] = prob_cost
        self.edges = edges
    ...
    def unify_mdpst(self):
        ...
        for (f_node, t_node) in edges.keys():
            ...
            for u in self.U:
                if u in list(prop.keys()):
                    edges_prob[(f_node, u, t_node)] = prop[u][0]
        self.edges_prob = edges_prob
        print('Unify MDPST Done')

class Motion_Prod_MDPST:
    # ----construct probabilistic-labeled MDP----
    def __init__(self, node, edge_dict, state_action_dict, U):
        self.name='motion_Prod_MDPST',
        self.U = U
        self.prod_nodes = node
        self.prod_state_action = state_action_dict
        self.add_edges(edge_dict)
        print("-------Motion MDPST Initialized-------")
        print("%s states and %s edges" %
    ...
    def add_edges(self, edge_dict):
        ...
        for edge, attri in edge_dict.items():
            ...
            if (f_node, t_node_set) in edges.keys():
                ...
                prob_cost[tuple(u)] = [attri[0], attri[1]]
                edges[(f_node, t_node_set)] = prob_cost
        self.prod_edges = edges
        self.prod_state_action_state = state_action_state


# ----------------------------------------------------------------------
#--------------Optimal policy synthesis for Product MDPST--------------------------
def syn_full_plan_mdpst(mdpst, Acc, SR):
    ...
    opt_action, v_new = optimal_plan_prefix_mdpst(mdpst, Acc, SR)
    plan_prefix = opt_action
...
def optimal_plan_prefix_mdpst(mdp, Acc, SR):
    ...
    for node in mdp.prod_nodes:
        if node not in sf:
            ...
            num_suffix += 1
            v_old[node] = 1
    print('Number of suffix states: %s' %num_suffix)
    ...
    delta_old = 1
    accuracy = 0.01
    while delta_old >= accuracy and num_iteration < 200:
        print('Number of interation: %s' %num_iteration)
        v_new, opt_action, delta_new = optimal_prefix_value_iteration_mdpst(mdp, Acc, SR, v_old)
        for s in SR:                    
            v_old[s] = v_new[s]
        for s in sf:
            v_old[s] = 1
...
def optimal_prefix_value_iteration_mdpst(mdpst, Acc, Sr, v_old):
    num1 = len(Sr)
    U = mdpst.U
    v_new = dict()
    num2 = len(U)
    ...
    for idx, s in enumerate(Sr):
        for idu, u in enumerate(U):
            if (s, u) in mdpst.prod_state_action.keys():
                t_set_set = mdpst.prod_state_action[(s, u)]
                #print(t_set_set)
                for k, t_set in enumerate(t_set_set):
                    #print(t_set)
                    pe = mdpst.prod_state_action_state[(s, u, t_set)]
                    #print(pe)
                    v_group = []
                    for t_node in t_set:
                        if t_node in list(v_old.keys()): 
                            if v_old[t_node]:                
                                v_group.append(v_old[t_node])
                        else:
                            print('error')
    ...
    #print(vlist)
    for idx, s in enumerate(Sr):
        v_new[s] =  max(vlist[idx])
        index = [i for i, value in enumerate(vlist[idx]) if value == v_new[s]]
        u_valid = set()
        for idu, u in enumerate(U):
            if idu in index:
                u_valid.add(u)
        opt_action[s] = u_valid
...
class Product_MDPST:
                        ...
                        if truth:
                            ...
                            for t_mdp_node in t_mdp_node_set:
                                for t_mdp_label, t_label_prob in smdp.nodes[t_mdp_node].items():
                                    ...
                                    t_prod_node_set += (tuple(t_prod_node),)
                            prob_cost = dict()
                            for u, attri in smdp.edges[(f_mdp_node, t_mdp_node_set)].items():
                                if attri[0] != 0:
                                    prob_cost[u] = (attri[0], attri[1])
                            if list(prob_cost.keys()):
                                prod_edges[(f_prod_node, t_prod_node_set)]= prob_cost                                   
        self.prod_edges = prod_edges

    def add_state_action_dict(self, smdp, dra):
        prod_state_action = dict()
        prod_state_action_state = dict()
        for fnode in self.prod_nodes:
            for u in self.U:
                prod_state_action[(fnode, u)] = set()

        # ----construct full product----
        for (f_prod_node, t_prod_node_set) in list(self.prod_edges.keys()):
            for u, attri in self.prod_edges[(f_prod_node, t_prod_node_set)].items():
                prod_state_action[(f_prod_node, u)].add(tuple(t_prod_node_set))
                prod_state_action_state[(f_prod_node, u, t_prod_node_set)] = attri[0]
        self.prod_state_action = prod_state_action
        self.prod_state_action_state = prod_state_action_state

        succ = dict()
        for f_prod_node in self.prod_nodes:
            tnode_list = set()
            for u in self.U:
                if (f_prod_node, u) in self.prod_state_action.keys():
                    t_prod_node_set = self.prod_state_action[(f_prod_node, u)]
                    for tnode_set in t_prod_node_set:
                        tnode_list.add(tnode_set)
            succ[f_prod_node] = tnode_list
        self.successors = succ

    def execution(self, smdp, pre_node, best_prefix_plan, WR, suffix_action, total_T, state_seq, label_seq):
        ...
        PX = []
        m = 0
        index_suffix_node = dict()
        for node in WR:
            index_suffix_node[node] = 0
        # ----
        while (t <= total_T):
            if (t == 0):
                print('---initial run----')
                ...
                label = label_seq[0]
                initial_set = self.prod_init.copy()
                ...
                error = True
                for next_state_set in self.successors[prev_state]:
                    if ((prev_state, u, next_state_set) in list(self.prod_state_action_state.keys())):
                        f = len(next_state_set)
                        ran_num = random.randint(0, f)
                        next_state = next_state_set[ran_num]
                        if next_state[0] == mdp_state and next_state[1] == label:
                ...
                if m != 2:  # in prefix or suffix
                    if prev_state in list(self.successors.keys()):
                        for next_state_set in self.successors[prev_state]:
                            if ((prev_state, u, next_state_set) in list(self.prod_state_action_state.keys())):
                                prob = self.prod_state_action_state[(prev_state, u, next_state_set)]
                                f = len(next_state_set)
                                ran_num = random.randint(0, f)
                                next_state = next_state_set[ran_num]
            ...
            # ----
            u, m, index_suffix_node = self.act_by_plan_mdpst(pre_node, best_prefix_plan, WR, suffix_action, current_state, index_suffix_node)
            #print('action: %s' %u)
            X.append(mdp_state)
    ...
    def act_by_plan_mdpst(self, prefix_node, best_prefix_plan, suffix_node, suffix_action, prod_state, index_suffix_node):
        ...
        plan_prefix = prefix_node
        plan_suffix = suffix_node
        if (prod_state in plan_prefix):
            print('In prefix')
            U_valid = best_prefix_plan[prod_state]
            #print(U_valid)
            U_list = list(U_valid)               
            l = len(U_valid)
            ran_num = random.randint(0, l)
            U = U_list[ran_num]
            #print(U)
            return U, 0, index_suffix_node
        elif (prod_state in plan_suffix):
            print('In suffix')
            U_valid = suffix_action[prod_state]
            #print(U_valid)
            U_list = list(U_valid)
            l = len(U_valid)
            num = index_suffix_node[prod_state]
            if num < l:
                U = U_list[num]
                index_suffix_node[prod_state] = num+1
            else:
                index_suffix_node[prod_state] = 0
                U = U_list[0]
            return U, 1, index_suffix_node
        else:
            print('Bad state')
            # for u in self.U:
            #     if (prod_state, u) in self.prod_state_action.keys():
            #         U = u
            #         break
            #     else:
            #         U = []
            U = random.choice(list(self.U)) 
            if U:          
                return U, 2, index_suffix_node
            else:
                return None, 2, index_suffix_node  
