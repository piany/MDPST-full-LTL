...
def visualize_run(XX, YY, name=None):
    ...
    for k, L in enumerate(LL):
        ...
        for n, l in enumerate(L):
            ...
            if ip_count >= 1:
                ...
                break
    print('Analyze done')
    print('Total %s simulations: %s failure (%s) (%s prefix, %s suffix), %s prefix successful; %s suffix successful' % (str(N), str(failure_count), str(failure_count/N),
          str(prefix_failure_count), str(suffix_failure_count), str(prefix_suc_count), str(suffix_suc_count)))
...
def visualize_state_dynamic(motion_mdp, WS_d, WS_node_dict, x, l):
...
def run_movie(motion_mdp, WS_d, WS_node_dict, X, L):
