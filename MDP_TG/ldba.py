...
class LDBA(DiGraph):
    def __init__(self, formula):
        ...
        acc_guard_string = {}
        tran_dict = dict()
        for num, ddict in enumerate(acc_dict):
            tran_dict = delta[num]
            label_set = set()
            for label, index in ddict.items():
                if index == [True]:
                    label_set.add(label)
            ...
            if label_set:
                acc.append(num)
                for l in label_set:
                    for num_t in range(statenum):
                        if (l, num_t) in tran_dict.items():
                            tran = (num, frozenset(l), num_t)
                            acc_tran.append(tran)
        ...
        self.graph['accept_tran'] = acc_tran
...
class Product_Ldba(DiGraph):
    ...
    def build_full(self):
        ...
        print("%s states, %s edges and %s accepting transitions" % (
            str(len(self.nodes())), str(len(self.edges())), str(len(self.graph['accept_tran']))))
    ...
    def build_acc_tran(self):
        acc_tran_product = set()
        acc_final_state = set()
        for f_node in self.nodes():
            f_mdp_label=f_node[1] 
            f_ldba_node=f_node[2]
            for t_node in self.successors(f_node): 
                t_ldba_node=t_node[2]
                tran = (f_ldba_node, f_mdp_label, t_ldba_node)
                #print(tran)
                if tran in self.graph['ldba'].graph['accept_tran']:
                    prod_mdp_edge = self[f_node][t_node]
                    for u, attri in prod_mdp_edge['prop'].items():
                        tran_prod = (f_node, u, t_node)
                        acc_tran_product.add(tran_prod) 
                        acc_final_state.add(tuple(tran_prod[2]))   
        self.graph['accept_tran'] = acc_tran_product
        self.graph['accept_final_state'] = acc_final_state
        print(acc_final_state)
        #print('Accepting transitions: %s' %str(acc_tran_product))

    def compute_S_f(self, Sneg):
        # ----find all accepting End components----
        S = Sneg
        Ip = self.graph['accept_final_state']
        S_f = []
        k = 1
        ...
        for T in MEC:
            ...
            if common:
                ...
                for fnode in T:
                    for tnode in common:
                        if tnode in self.successors(fnode):
                            prod_mdp_edge = self[fnode][tnode]
                            for u, attri in prod_mdp_edge['prop'].items():
                                if u in Act[fnode]:
                                    tran = (fnode, u, tnode)
                                    if tran in self.graph['accept_tran']:
                                        print(tran)
                                        common_tran.add(tran)
                print(len(common_state))
                print(len(common_tran))

                if common_tran:
                    if len(T) > 1:
                        S_fi.append([T, common_state, common_tran, Act])
                    ...
                    if len(T) == 1:  # self-loop
                        ...
                        S_fi.append([T, common_state, common_tran, loop_act])
