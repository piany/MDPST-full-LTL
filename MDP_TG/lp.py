...
def syn_plan_prefix(prod_mdp, MEC, gamma):
    ...
    f_plan_prefix_act = open('to_matlab/data/plan_prefix_act.dat','w')
    f_plan_prefix_prob = open('to_matlab/data/plan_prefix_prob.dat','w')
    ...
    for init_node in prod_mdp.graph['initial']:
        ...
        for np in list(Sr):
            f_prod_Sr.write('%s, %s, %s\n' %(np[0][0], np[0][1], np[2]))
        for hp in list(sf):
            f_prod_Sf.write('%s, %s, %s\n' %(hp[0][0], hp[0][1], hp[2])) 
        # ---------solve lp------------
        print('-----')
...
def syn_plan_suffix(prod_mdp, MEC, y_in_sf):
    ...
    f_prod_suffix_Sn = open('to_matlab/data/prod_suffix_Sn.dat','w')
    f_plan_suffix_dict = open('to_matlab/data/plan_suffix.dat','w')
    f_plan_suffix_act = open('to_matlab/data/plan_suffix_act.dat','w')
    f_plan_suffix_prob = open('to_matlab/data/plan_suffix_prob.dat','w')
    sf = MEC[0]
    ip = MEC[1]
    for lip in list(ip):
        f_prod_suffix_Ip.write('%s, %s, %s, %s\n' %(lip[0][0], lip[0][1], lip[0][2], lip[2]))
    act = MEC[2].copy()
    delta = 0.01
    gamma = 0.00
    for init_node in prod_mdp.graph['initial']:
        ...
        for lsn in list(Sn):
            f_prod_suffix_Sn.write('%s, %s, %s, %s\n' %(lsn[0][0], lsn[0][1], lsn[0][2], lsn[2]))
...
def syn_plan_bad(prod_mdp, state_types):
    f_plan_bad_dict = open('to_matlab/data/plan_bad.dat','w')
    f_plan_bad_act = open('to_matlab/data/plan_bad_act.dat','w')
    f_plan_bad_prob = open('to_matlab/data/plan_bad_prob.dat','w')
    ...
    for sd in Sd:
        ...
        proj_cost = dict()
        postqf = prod_mdp.graph['ldba'].successors(qf)
                    ...
                    for lt in label.keys():
                        ...
                        for qt in postqf:
                            if (xt, lt, qt) in Sf.union(Sr):
                                dist[qt] = prod_mdp.graph['ldba'].check_distance_for_dra_edge(
                                    lf, qf, qt)
                        if list(dist.keys()):
...
def act_by_plan(best_plan, prod_state):
...
def rd_act_by_plan(best_plan, prod_state, I):
    # ----choose the randomized action by the optimal policy----
